...
async def shop_lootbox(ctx: Context, user: discord.abc.User):
    text = texts[lang]["lootbox_select"]
    ...
    with lock:
        async with database.new_session() as session:
            ...
            if(money.get_money_user(player) < cout):
                await ctx.send(texts[lang]["lootbox_not_enough_money"].format(user = user.name, money_emoji = money.MONEY_EMOJI, cout = cout))
            ...
            if(interact_game.nb_bully_in_team(player) >= interact_game.BULLY_NUMBER_MAX):
                await ctx.send(texts[lang]["max_bullies_reached"].format(user = user.name, max_bullies = interact_game.BULLY_NUMBER_MAX))
            ...
            text_lootbox = bully.mise_en_forme_str(texts[lang]["lootbox_purchase_success"].format(user = user.name, bully = b.name, rarity = b.rarity.name))
